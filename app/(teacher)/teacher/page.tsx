...
export default function TeacherOverview() {
  ...
  const activities = mockData.teacherActivities.slice(0, 4);
  const students = mockData.teacherStudents;

  const [notifications, setNotifications] = useState(mockData.teacherNotifications);
  const [selectedActivity, setSelectedActivity] = useState<string | null>(null);

  const activeStudents = students.filter(s => s.status === 'active').length;
  const completionRate = Math.round(students.reduce((acc, s) => acc + (s.modulesCompleted / s.totalModules) * 100, 0) / students.length);
  ...
  return (
    <div className="p-6 md:p-12 space-y-10 font-sans max-w-5xl mx-auto h-full">
      ...
      <div className="relative">
        ...
        <p className="text-gray-500 font-medium relative z-10">Berikut merupakan ringkasan aktivitas murid di <span className="font-bold text-teal-600">{institution.name}</span> hari ini.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 relative z-10">
        <Link href="/teacher/students">
          <motion.div 
            ...
            whileHover={{ y: -4 }}
            className="bg-gradient-to-br from-teal-500 to-emerald-600 text-white rounded-[2rem] p-6 shadow-xl shadow-teal-500/30 flex flex-col relative overflow-hidden cursor-pointer group"
          >
        ...
        <Link href="/teacher/students">
          <motion.div 
            ...
            whileHover={{ y: -4 }}
            className="bg-white rounded-[2rem] p-6 shadow-xl shadow-gray-200/50 border border-gray-100 flex flex-col cursor-pointer group"
          >
            ...
            <div className="flex-1">
              <div className="text-4xl font-black mb-1 text-gray-900">{activeStudents}</div>
        ...
        >
          ...
          <div className="flex-1">
            <div className="text-4xl font-black mb-1 text-gray-900">{completionRate}%</div>
