      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-4">
        <div className="flex-1 bg-[#0A101C]/80 backdrop-blur-md border border-white/5 rounded-[1.2rem] px-5 py-4 flex items-center gap-3 shadow-inner group focus-within:border-white/20 transition-all">
          ...
          <input 
            ...
            autoComplete="off"
            className="bg-transparent border-none text-white text-[15px] focus:outline-none w-full placeholder:text-gray-600 font-medium" 
          />
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
           <button 
             onClick={() => alert('Advanced Filter Menu: Coming Soon')}
             className="flex-1 sm:flex-none justify-center bg-[#0A101C] border border-white/5 px-5 py-4 rounded-[1.2rem] text-gray-400 hover:text-white hover:bg-white/5 transition flex items-center gap-2 text-[12px] font-bold uppercase"
           >
              <Filter className="w-4 h-4" /> Filter
           </button>
           <button 
             onClick={() => setIsModalOpen(true)}
             className="flex-1 sm:flex-none justify-center bg-red-600 px-6 py-4 rounded-[1.2rem] text-white hover:bg-red-500 hover:-translate-y-0.5 transition-all text-[12px] font-bold uppercase shadow-[0_0_20px_rgba(220,38,38,0.3)] flex items-center gap-2"
           >
                ...
                >
                  ...
                  <td className="px-8 py-5 text-right whitespace-nowrap">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition mr-2">
                       <button onClick={() => alert(`View details for ${user.name}`)} className="p-2.5 text-gray-400 hover:text-white transition hover:bg-white/10 border border-transparent hover:border-white/10 rounded-xl"><Eye className="w-4 h-4" /></button>
                       <button onClick={() => alert("Open context menu")} className="p-2.5 text-gray-400 hover:text-red-400 transition hover:bg-white/10 border border-transparent hover:border-white/10 rounded-xl"><MoreVertical className="w-4 h-4" /></button>
                    </div>
                  </td>
                </motion.tr>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2 block" htmlFor="userName">Full Name</label>
                <input id="userName" name="userName" type="text" autoComplete="off" placeholder="e.g. Budi Santoso" className="w-full bg-[#070B14] border border-gray-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-red-500/50" />
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2 block" htmlFor="userEmail">Email Address</label>
                <input id="userEmail" name="userEmail" type="email" autoComplete="off" placeholder="budi@example.com" className="w-full bg-[#070B14] border border-gray-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-red-500/50" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2 block">Role</label>
                ...
                <div>
                  <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2 block">Institution</label>
                  <select className="w-full bg-[#070B14] border border-gray-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-red-500/50 appearance-none">
                    {mockData.institutions.map(inst => <option key={inst.id} value={inst.id}>{inst.name}</option>)}
                  </select>
                </div>
              </div>
            </div>
            <div className="mt-8 flex justify-end gap-3">
              ...
              <button 
                onClick={() => {
                  alert("User created successfully! (Demo Simulation)");
