...
import { Star, Play, PlayCircle, BookOpen, ShieldAlert, Sparkles, Video, BookText, Image as ImageIcon } from 'lucide-react';
...
export default function ChildDashboardPage() {
  ...
  const digfoModules = modules.filter(m => m.type === 'digfo');
  const digviModules = modules.filter(m => m.type === 'digvi');
  const emodulModules = modules.filter(m => m.type === 'e-modul');

  interface ModuleCardProps {
    mod: any; // Type according to your mock data structure as needed
  ...
  const ModuleCard = ({ mod, icon, bgClass, borderClass, textClass, badgeClass }: ModuleCardProps) => {
    const category = categories.find(c => c.id === mod.category_id);
    return (
      <motion.div
        whileHover={{ scale: 1.02, y: -2 }}
        whileTap={{ scale: 0.98, y: 2 }}
        onClick={() => router.push(`/child/module/${mod.id}`)}
      ...
      >
        ...
        <div className="flex justify-end mt-2">
          <div className="bg-white/90 text-gray-800 font-bold text-xs py-2 px-4 rounded-full flex items-center gap-2 shadow-sm font-sans">
            Mulai Belajar <Play className="w-3 h-3 fill-current" />
          </div>
        </div>
      </motion.div>
    );
  ...
  return (
    <div className="p-6 md:p-10 pt-12 pb-32 space-y-12 font-grandstander max-w-[1400px] mx-auto">
      ...
      >
        ...
        <div className="absolute -bottom-4 -right-4 w-32 h-32 md:w-48 md:h-48 bg-orange-300 rounded-full opacity-50 flex items-center justify-center">
        </div>
      </motion.div>

      {/* Gamification Bar */}
      <motion.div 
        onClick={() => router.push('/child/hall-of-fame')}
      ...
      >
        ...
        <div className="h-5 bg-gray-100 rounded-full overflow-hidden shadow-inner">
          <motion.div 
            initial={{ width: 0 }}
            animate={{ width: `${(child.total_points / 500) * 100}%` }}
