  ...
  return (
    <div className="font-sans text-gray-100 flex flex-col gap-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Total Users', value: '12,842', change: '+12%', icon: <Users className="w-5 h-5 text-red-400" /> },
          { label: 'Active Children', value: '8,421', change: '+8%', icon: <TrendingUp className="w-5 h-5 text-indigo-400" /> },
          { label: 'Institutions', value: '156', change: '+4%', icon: <Globe className="w-5 h-5 text-emerald-400" /> },
          { label: 'Modules Completed', value: '45,210', change: '+24%', icon: <BarChart3 className="w-5 h-5 text-amber-400" /> },
        ].map((stat, i) => (
          <motion.div
            key={i}
          ...
          <div className="h-64 flex items-end gap-3 px-2">
            {[40, 70, 45, 90, 65, 80, 100].map((h, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-3">
                ...
                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">D{i+1}</span>
              </div>
            ))}
          </div>
        </div>
