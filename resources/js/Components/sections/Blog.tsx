...
const Blog: React.FC<BlogProps> = ({ articles: articlesProp }) => {
  const articleList = Array.isArray(articlesProp)
    ? articlesProp
    : ((articlesProp as any)?.data || mockData.articles.slice(0, 2));
  ...
  return (
    <section id="blog">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {articleList.slice(0, 2).map((article: any) => {
          const categoryColor = article.category_color || article.categoryColor || 'orange';
          ...
          return (
            ...
            >
              <div className={`w-full sm:w-1/3 h-40 sm:h-auto bg-${categoryColor}-100 rounded-2xl overflow-hidden relative`}>
                <img 
                  src={imageUrl} 
                  alt={article.title} 
              ...
              <div className="w-full sm:w-2/3 py-2 flex flex-col justify-center">
                <span className={`text-[10px] font-bold text-${categoryColor}-500 bg-${categoryColor}-50 w-fit px-3 py-1 rounded-full tracking-widest uppercase mb-3 border border-${categoryColor}-100`}>
                  {article.category}
                </span>
